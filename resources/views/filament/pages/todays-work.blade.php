            ...
            <div class="mt-4 grid gap-3 md:grid-cols-3">
                <div class="rounded-2xl border border-violet-200 bg-violet-50 p-4 dark:border-violet-900 dark:bg-violet-950/30">
                    ...
                    <div class="mt-2 text-3xl font-black text-violet-950 dark:text-violet-100">
                        {{ number_format((int) ($parcelMovement['confirmed_waiting_dispatch_count'] ?? 0)) }}
                    </div>
                    <div class="mt-1 text-sm font-semibold text-violet-900/80 dark:text-violet-100/80">
                        {{ number_format((int) ($parcelMovement['confirmed_waiting_dispatch_live_count'] ?? 0)) }} live in Stock App
                    </div>
                </div>
        ...
        <x-filament::section>
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="text-sm font-black text-gray-700 dark:text-gray-200">
                    Showing {{ number_format(count($parcelMovement['confirmed_waiting_dispatch_items'] ?? [])) }} parcel(s)
                </div>
            </div>

            <div class="mt-4 overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950">
                <div class="grid gap-3 border-b border-gray-200 bg-gray-50 px-4 py-3 text-xs font-black uppercase tracking-wide text-gray-500 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 lg:grid-cols-[1.2fr_1.2fr_0.8fr_0.7fr_0.8fr_0.9fr_1fr]">
                    <span>Parcel</span>
                ...
                <div class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse (($parcelMovement['confirmed_waiting_dispatch_items'] ?? []) as $item)
                        <div class="grid gap-3 px-4 py-4 text-sm lg:grid-cols-[1.2fr_1.2fr_0.8fr_0.7fr_0.8fr_0.9fr_1fr]">
                            ...
                            <div class="min-w-0">
                                <div class="font-semibold text-gray-900 dark:text-gray-100">
                                    {{ $item['customer'] ?? 'Customer not shown' }}
                                </div>
                                <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                    {{ $item['phone'] ?? 'Phone not shown' }}
                                </div>
                            </div>
                            ...
                            <div class="font-semibold capitalize text-gray-700 dark:text-gray-200">
                                {{ $item['status'] ?? 'confirmed' }}
                            </div>

                            <div class="text-sm text-gray-600 dark:text-gray-300">
                                {{ $item['next_action'] ?? 'Add tracking and send this parcel to courier.' }}
                            </div>
                        </div>
                    @empty
