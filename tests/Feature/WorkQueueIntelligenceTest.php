...
class WorkQueueIntelligenceTest extends TestCase
{
    ...
    public function test_employee_work_screen_renders(): void
    {
        ...
        $this->actingAs($employee)
            ...
            ->assertDontSee('Manager control')
            ->assertDontSee('Floor Worker, focus the team')
            ->assertDontSee('You report to')
            ->assertDontSee('Your direct team')
            ->assertDontSee('Your three actions today')
            ->assertDontSee('Manager target recovery')
            ->assertDontSee('Monthly company target gap')
            ->assertDontSee('Additional deliveries required')
            ->assertDontSee('Managers see only the remaining operational gap')
    ...
    public function test_dispatch_employee_sees_parcel_movement_for_current_business(): void
    {
        ...
        OperationalEvent::query()->create([
            ...
            'revenue_amount' => 0,
            'payload' => ['order_id' => 'ORDER-PENDING-CONFIRMATION', 'sale_amount' => 800, 'status' => 'Pending'],
        ...
        OperationalEvent::query()->create([
            ...
            'event_type' => OperationalEvent::ORDER_CONFIRMED,
            'external_id' => 'ORDER-CONFIRMATION-PENDING-NESTED',
            'revenue_amount' => 0,
            'payload' => ['order_id' => 'ORDER-CONFIRMATION-PENDING-NESTED', 'sale_amount' => 600, 'data' => ['status' => 'Confirmation Pending']],
    ...
    public function test_employee_parcel_movement_dedupes_mixed_order_identifiers(): void
    {
        ...
        $employee = User::query()->create([
            'name' => 'Arafath',
            'email' => 'identifier-mix@example.com',
            'password' => Hash::make('password'),
            'business_id' => $business->id,
            'is_employee' => true,
            'is_platform_admin' => false,
        ...
        OperationalEvent::query()->create([
            ...
            'source' => 'stock_app',
            'event_type' => OperationalEvent::ORDER_CONFIRMED,
            'external_id' => 'ORDER-MIXED-1-confirmed',
            'revenue_amount' => 0,
            'payload' => ['order_id' => 'ORDER-MIXED-1', 'sale_amount' => 2500],
        ...
        OperationalEvent::query()->create([
            ...
            'source' => 'stock_app',
            'event_type' => OperationalEvent::TRACKING_NUMBER_ADDED,
            'external_id' => 'ORDER-MIXED-1-tracking_number_added',
            'revenue_amount' => 0,
            'payload' => ['order_number' => 'ORDER-MIXED-1', 'sale_amount' => 2500],
            'occurred_at' => now()->subMinutes(5),
        ]);
    ...
    public function test_client_owner_dashboard_shows_the_simple_parcel_profit_boxes(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertSee('Expenses entered')
            ->assertSee('Open expenses')
            ->assertSee('Staff salary pressure')
            ->assertSee('Open staff')
